...
export default function InfoBox({ info }) {
  ...
  useEffect(() => {
    const forcast = async () => {
      const res = await fetch(
        `${import.meta.env.VITE_FORCAST_URL}q=${info.city}&appid=${
  ...
  return (
    <>
          ...
          <Card sx={{ maxWidth: 345 }}>
            ...
            <CardContent>
              ...
              >
                ...
                <p>
                  Weather can de described as{" "}
                  <i className="dark">{info.weather}</i> and feels like{" "}
                  <span className="dark">{info.feelsLike} &deg;C</span>
                </p>
              </Typography>
            </CardContent>
          </Card>
      ...
      {forcastData === null ? "" : <Forcast data={forcastData} />}
    </>
  );
}
